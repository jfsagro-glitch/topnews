...
### Глобальная остановка сбора (только sandbox)
- Флаг стопа хранится в Redis: `jur:stop:global:sandbox` (значение `1`).
- TTL по умолчанию: 3600 сек, после чего стоп снимается автоматически.
- В prod флаг игнорируется, сбор никогда не останавливается из-за него.
- Переменная `GLOBAL_COLLECTION_STOP_FILE` считается устаревшей и игнорируется.
